...
bool ChessBoard :: isOccupied(int _row, int _col, bool silent){
    ...
    for(int p=0; p<32; p++){
        ...
        if ((pieces[p]->row == _row) && (pieces[p]->col == _col)) { // there is a piece here already
            if (!silent) std::cout << "isOccupied()  rc = (" << _row << ", " << colNames[_col]  << ") is occupied by " << pieces[p]->name <<std::endl;
            return true;
        }
    }
...
bool ChessBoard :: takeTurn(string pieceName, int row, int col, bool silent ){
    ...
    for(int p=0; p<32; p++){ // look for the piece we are trying to move amongst all pieces[] pointers
      ...
      if (pieces[p]->name == pieceName) { // found the piece
        ...
        if ( isKingInCheck( whiteTurn, false ) )  // the successful move has put the king of the new-current turn's color is in check  (not silent)
...
// WIP: return true if the King of the color specified is currently in check
bool ChessBoard :: isKingInCheck(bool isWhite, bool silent  ) {

  // TODO: handle case where we must capture in a "what if" move before we call this functin, to determine if the threat has been eliminated
  ...
  for (int i=offset; i<16+offset; i++) { // check all the white pieces if this move is for a black piece, else check the black pieces
    ...
    if ( enemyPiece->isLegalMoveTo( thisKing->row, thisKing->col ) ) { // if legal for this enemy piece to move to this king, then the m
